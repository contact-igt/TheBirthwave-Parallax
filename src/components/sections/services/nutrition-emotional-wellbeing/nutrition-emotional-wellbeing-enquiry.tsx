...
import { nutritionEnquiry, nutritionMobileCta } from "@/content/nutrition-emotional-wellbeing-landing-content";
 ...
 * this page targets. TODO(infra): exactly like the other nine landing
 ...
 */
export function NutritionEnquiry() {
  const nameId = useId();
  ...
  // Broadcasts focus state on the form so `NutritionMobileEnquiryBar` can
  ...
  return (
    <section id="enquiry" aria-labelledby="nutrition-enquiry-heading" className="relative isolate scroll-mt-[var(--header-height)] bg-[#e8cec2] section-pad">
      <NutritionReveal className="container-birthwave grid gap-12 lg:grid-cols-[1fr_1fr] lg:items-start lg:gap-20">
        <div className="max-w-md">
          <p className="eyebrow">{nutritionEnquiry.eyebrow}</p>
          <h2 id="nutrition-enquiry-heading" className="mt-4 text-[clamp(2rem,1.5rem+2.6vw,3.4rem)] leading-[1.1] font-semibold tracking-[var(--tracking-tight)] text-ink">
            {nutritionEnquiry.heading}
          </h2>
          <p className="mt-5 text-lg leading-[var(--leading-relaxed)] text-ink-soft">{nutritionEnquiry.body}</p>
          <p className="mt-8 border-t border-[var(--color-border)] pt-4 text-sm leading-relaxed text-ink-soft">{nutritionEnquiry.urgentNote}</p>
        </div>

        <div className="w-full max-w-md rounded-sm bg-paper p-7 sm:p-8">
          <p className="mb-5 font-body text-xs font-semibold tracking-[var(--tracking-wider)] text-terracotta-deep uppercase">{nutritionEnquiry.previewNotice}</p>

          {status === "previewed" ? (
            <p role="status" className="text-base leading-[var(--leading-relaxed)] text-ink-soft">
              {nutritionEnquiry.previewResult}
            </p>
          ) : (
            <form ref={formRef} onSubmit={handleSubmit} noValidate className="flex flex-col gap-5">
              <div className="flex flex-col gap-1.5">
                <label htmlFor={nameId} className={labelClass}>
                  {nutritionEnquiry.fields.name}
                </label>
                <input
                  id={nameId}
                  name="name"
              ...
              <div className="flex flex-col gap-1.5">
                <label htmlFor={contactId} className={labelClass}>
                  {nutritionEnquiry.fields.contact}
                </label>
                <input
                  id={contactId}
                  name="contact"
              ...
              <div className="flex flex-col gap-1.5">
                <label htmlFor={messageId} className={labelClass}>
                  {nutritionEnquiry.fields.message}
                </label>
              ...
              <p className="text-xs leading-relaxed text-ink-soft">{nutritionEnquiry.formNote}</p>
 ...
 * `#nutrition-hero`/`#enquiry` ids.
 */
export function NutritionMobileEnquiryBar() {
