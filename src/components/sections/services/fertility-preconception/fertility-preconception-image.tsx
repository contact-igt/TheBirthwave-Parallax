...
import { fertilityImagery, type FertilityImageKey } from "@/content/fertility-preconception-landing-imagery";
...
/**
 * Editorial image wrapper for Fertility & Preconception landing page.
 ...
 */
export function FertilityImage({
  imageKey,
  corner = "tr",
  aspect = "aspect-[4/5]",
  className,
  ...
  const image = fertilityImagery[imageKey];

  if (!("src" in image) || !image.src) {
    return (
      <MediaPlaceholder
        alt={image.alt}
        gradient={"gradient" in image && typeof image.gradient === "string" ? image.gradient : "linear-gradient(135deg, var(--color-paper-dim), var(--color-terracotta))"}
        corner={corner}
        aspect={aspect}
        className={className}
      />
