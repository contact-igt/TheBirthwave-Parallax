...
import { normalBirthEnquiry, normalBirthMobileCta } from "@/content/normal-birth-delivery-landing-content";
 ...
 * situation `pregnancy-antenatal-appointment.tsx` and
 * `birth-preparation-enquiry.tsx` document for their own forms. A
 * persistent "Preview" notice stays visible above the form at all times,
 * and submitting shows an honest "this is a design preview, no enquiry
 * has been sent" message — never a fake success/confirmation.
 *
 * Deliberately short, per the brief: full name, a single phone-or-email
 * contact field, and an optional message — no medical history or health
 * information, reinforced by the visible urgent-care note. Nothing here
 * is written to browser storage.
 */
export function NormalBirthEnquiry() {
  const nameId = useId();
  ...
  // Broadcasts focus state on the form so `NormalBirthMobileEnquiryBar` can
  // hide itself while a field is actively being edited — mirrors
  // `birth-preparation-enquiry.tsx`'s own effect.
  useEffect(() => {
  ...
  return (
    <section
      id="nbd-enquiry"
      aria-labelledby="nbd-enquiry-heading"
      className="relative isolate scroll-mt-[var(--header-height)] bg-[#e8cec2] section-pad"
    >
      <NormalBirthReveal className="container-birthwave grid gap-12 lg:grid-cols-[1fr_1fr] lg:items-start lg:gap-20">
        <div className="max-w-md">
          <p className="eyebrow">{normalBirthEnquiry.eyebrow}</p>
          <h2 id="nbd-enquiry-heading" className="mt-4 text-[clamp(2rem,1.5rem+2.6vw,3.4rem)] leading-[1.1] font-semibold tracking-[var(--tracking-tight)] text-ink">
            {normalBirthEnquiry.heading}
          </h2>
          <p className="mt-5 text-lg leading-[var(--leading-relaxed)] text-ink-soft">{normalBirthEnquiry.body}</p>
        </div>
            ...
            <form ref={formRef} onSubmit={handleSubmit} noValidate className="flex flex-col gap-5">
              ...
              <div className="flex flex-col gap-1.5">
                <label htmlFor={contactId} className={labelClass}>
                  {normalBirthEnquiry.fields.contact}
                </label>
                <input
                  id={contactId}
                  name="contact"
              ...
              <div className="flex flex-col gap-1.5">
                <label htmlFor={messageId} className={labelClass}>
                  {normalBirthEnquiry.fields.message}
                </label>
                <textarea
                  id={messageId}
                  name="message"
              ...
              <p className="text-xs leading-relaxed text-ink-soft">{normalBirthEnquiry.formNote}</p>
              ...
              <button type="submit" className={submitButtonClass}>
                {normalBirthEnquiry.submitLabel}
              </button>
            </form>
          )}
        </div>
 ...
 * focus. Mirrors `BirthPrepMobileEnquiryBar`/`PregnancyMobileAppointmentBar`
 * exactly, retargeted at this page's own `#nbd-hero`/`#nbd-enquiry` ids.
 */
export function NormalBirthMobileEnquiryBar() {
  const [pastHero, setPastHero] = useState(false);
  const [formVisible, setFormVisible] = useState(false);
  const [formEditing, setFormEditing] = useState(false);

  useEffect(() => {
    const hero = document.getElementById("nbd-hero");
    const form = document.getElementById("nbd-enquiry");
    if (!hero || !form) return;
  ...
  return (
    ...
    >
      <a
        href={normalBirthMobileCta.href}
