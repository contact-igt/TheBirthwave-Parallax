 ...
 * one-shot reveal as `NormalBirthReveal`/`BirthPrepReveal`. `useScrollReveal`
 ...
 * permanently invisible. The `vbac-reveal` class plus the single
 * `<noscript>` override in `vbac-landing-page.tsx` forces every instance
 * back to its settled, fully visible state when JS is unavailable.
 */
export function VbacReveal({ className, children }: { className?: string; children: ReactNode }) {
  ...
  return (
    <div
      ref={ref}
      className={cx(
        "vbac-reveal transition-[opacity,transform] duration-[var(--duration-base)] ease-[var(--ease-signature)] motion-reduce:translate-y-0 motion-reduce:opacity-100",
