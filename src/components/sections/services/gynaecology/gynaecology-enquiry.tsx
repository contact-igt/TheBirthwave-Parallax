...
import { gynaecologyEnquiry, gynaecologyMobileCta } from "@/content/gynaecology-landing-content";
...
/**
 * Section 10 — Final Enquiry. TODO(infra): exactly like the other six
 * landing pages' own forms, no backend endpoint or verified enquiry inbox
 ...
 */
export function GynaecologyEnquiry() {
  const nameId = useId();
  ...
  // Broadcasts focus state on the form so `GynaecologyMobileEnquiryBar` can
  // hide itself while a field is actively being edited — mirrors
  // `VaginismusEnquiry`'s own effect.
  useEffect(() => {
  ...
  return (
    <section id="gynaecology-enquiry" aria-labelledby="gynaecology-enquiry-heading" className="relative isolate scroll-mt-[var(--header-height)] bg-[#e8cec2] section-pad">
      <GynaecologyReveal className="container-birthwave grid gap-12 lg:grid-cols-[1fr_1fr] lg:items-start lg:gap-20">
        <div className="max-w-md">
          <p className="eyebrow">{gynaecologyEnquiry.eyebrow}</p>
          <h2 id="gynaecology-enquiry-heading" className="mt-4 text-[clamp(2rem,1.5rem+2.6vw,3.4rem)] leading-[1.1] font-semibold tracking-[var(--tracking-tight)] text-ink">
            {gynaecologyEnquiry.heading}
          </h2>
          <p className="mt-5 text-lg leading-[var(--leading-relaxed)] text-ink-soft">{gynaecologyEnquiry.body}</p>
          <p className="mt-8 border-t border-[var(--color-border)] pt-4 text-sm leading-relaxed text-ink-soft">{gynaecologyEnquiry.urgentNote}</p>
        </div>

        <div className="w-full max-w-md rounded-sm bg-paper p-7 sm:p-8">
          <p className="mb-5 font-body text-xs font-semibold tracking-[var(--tracking-wider)] text-terracotta-deep uppercase">{gynaecologyEnquiry.previewNotice}</p>

          {status === "previewed" ? (
            <p role="status" className="text-base leading-[var(--leading-relaxed)] text-ink-soft">
              {gynaecologyEnquiry.previewResult}
            </p>
          ) : (
            <form ref={formRef} onSubmit={handleSubmit} noValidate className="flex flex-col gap-5">
              <div className="flex flex-col gap-1.5">
                <label htmlFor={nameId} className={labelClass}>
                  {gynaecologyEnquiry.fields.name}
                </label>
                <input
                  id={nameId}
                  name="name"
              ...
              <div className="flex flex-col gap-1.5">
                <label htmlFor={contactId} className={labelClass}>
                  {gynaecologyEnquiry.fields.contact}
                </label>
                <input
                  id={contactId}
                  name="contact"
              ...
              <div className="flex flex-col gap-1.5">
                <label htmlFor={messageId} className={labelClass}>
                  {gynaecologyEnquiry.fields.message}
                </label>
              ...
              <button type="submit" className={submitButtonClass}>
                {gynaecologyEnquiry.submitLabel}
              </button>
            </form>
          )}
        </div>
 ...
 * the enquiry form is in view OR while any of its fields has focus.
 * Mirrors `VaginismusMobileEnquiryBar` exactly, retargeted at this page's
 * own `#gynaecology-hero`/`#gynaecology-enquiry` ids.
 */
export function GynaecologyMobileEnquiryBar() {
  const [pastHero, setPastHero] = useState(false);
  const [formVisible, setFormVisible] = useState(false);
  const [formEditing, setFormEditing] = useState(false);

  useEffect(() => {
    const hero = document.getElementById("gynaecology-hero");
    const form = document.getElementById("gynaecology-enquiry");
    if (!hero || !form) return;
  ...
  return (
    ...
    >
      <a
        href={gynaecologyMobileCta.href}
