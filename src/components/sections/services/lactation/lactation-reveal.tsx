 ...
 * one-shot reveal as `VbacReveal`/`GynaecologyReveal`. `useScrollReveal`
 ...
 * permanently invisible. The `lactation-reveal` class plus the single
 * `<noscript>` override in `lactation-landing-page.tsx` forces every
 * instance back to its settled, fully visible state when JS is unavailable.
 */
export function LactationReveal({ className, children }: { className?: string; children: ReactNode }) {
  ...
  return (
    <div
      ref={ref}
      className={cx(
        "lactation-reveal transition-[opacity,transform] duration-[var(--duration-base)] ease-[var(--ease-signature)] motion-reduce:translate-y-0 motion-reduce:opacity-100",
