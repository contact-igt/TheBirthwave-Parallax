...
import { postpartumEnquiry, postpartumMobileCta } from "@/content/postpartum-care-landing-content";
 ...
 * this page targets. TODO(infra): exactly like the other eight landing
 ...
 */
export function PostpartumEnquiry() {
  const nameId = useId();
  ...
  // Broadcasts focus state on the form so `PostpartumMobileEnquiryBar` can
  // hide itself while a field is actively being edited — mirrors
  // `LactationEnquiry`'s own effect.
  useEffect(() => {
  ...
  return (
    <section id="enquiry" aria-labelledby="postpartum-enquiry-heading" className="relative isolate scroll-mt-[var(--header-height)] bg-[#e8cec2] section-pad">
      <PostpartumReveal className="container-birthwave grid gap-12 lg:grid-cols-[1fr_1fr] lg:items-start lg:gap-20">
        <div className="max-w-md">
          <p className="eyebrow">{postpartumEnquiry.eyebrow}</p>
          <h2 id="postpartum-enquiry-heading" className="mt-4 text-[clamp(2rem,1.5rem+2.6vw,3.4rem)] leading-[1.1] font-semibold tracking-[var(--tracking-tight)] text-ink">
            {postpartumEnquiry.heading}
          </h2>
          <p className="mt-5 text-lg leading-[var(--leading-relaxed)] text-ink-soft">{postpartumEnquiry.body}</p>
          <p className="mt-3 text-base leading-[var(--leading-relaxed)] text-ink-soft">{postpartumEnquiry.bodySecondary}</p>
          <p className="mt-8 border-t border-[var(--color-border)] pt-4 text-sm leading-relaxed text-ink-soft">{postpartumEnquiry.urgentNote}</p>
        </div>

        <div className="w-full max-w-md rounded-sm bg-paper p-7 sm:p-8">
          <p className="mb-5 font-body text-xs font-semibold tracking-[var(--tracking-wider)] text-terracotta-deep uppercase">{postpartumEnquiry.previewNotice}</p>

          {status === "previewed" ? (
            <p role="status" className="text-base leading-[var(--leading-relaxed)] text-ink-soft">
              {postpartumEnquiry.previewResult}
            </p>
          ) : (
            <form ref={formRef} onSubmit={handleSubmit} noValidate className="flex flex-col gap-5">
              <div className="flex flex-col gap-1.5">
                <label htmlFor={nameId} className={labelClass}>
                  {postpartumEnquiry.fields.name}
                </label>
                <input
                  id={nameId}
                  name="name"
              ...
              <div className="flex flex-col gap-1.5">
                <label htmlFor={contactId} className={labelClass}>
                  {postpartumEnquiry.fields.contact}
                </label>
                <input
                  id={contactId}
                  name="contact"
              ...
              <div className="flex flex-col gap-1.5">
                <label htmlFor={messageId} className={labelClass}>
                  {postpartumEnquiry.fields.message}
                </label>
              ...
              <p className="text-xs leading-relaxed text-ink-soft">{postpartumEnquiry.formNote}</p>
 ...
 * `LactationMobileEnquiryBar` exactly, retargeted at this page's own
 * `#postpartum-hero`/`#enquiry` ids.
 */
export function PostpartumMobileEnquiryBar() {
  const [pastHero, setPastHero] = useState(false);
  const [formVisible, setFormVisible] = useState(false);
  const [formEditing, setFormEditing] = useState(false);

  useEffect(() => {
    const hero = document.getElementById("postpartum-hero");
