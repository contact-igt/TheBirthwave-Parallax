 ...
 * one-shot reveal as `LactationReveal`/`VbacReveal`. `useScrollReveal`
 ...
 * permanently invisible. The `postpartum-reveal` class plus the single
 * `<noscript>` override in `postpartum-care-landing-page.tsx` forces every
 * instance back to its settled, fully visible state when JS is
 * unavailable.
 */
export function PostpartumReveal({ className, children }: { className?: string; children: ReactNode }) {
  ...
  return (
    <div
      ref={ref}
      className={cx(
        "postpartum-reveal transition-[opacity,transform] duration-[var(--duration-base)] ease-[var(--ease-signature)] motion-reduce:translate-y-0 motion-reduce:opacity-100",
