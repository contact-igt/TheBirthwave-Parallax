...
import { naturalBirthEnquiry, naturalBirthMobileCta } from "@/content/natural-birth-landing-content";
...
/**
 * Section 11 — Start With Your Questions. `id: enquiry` — this page's own
 * explicit anchor, targeted by every CTA above. TODO(infra): exactly like
 * `vbacEnquiry`, no backend endpoint or verified enquiry inbox exists
 ...
 */
export function NaturalBirthEnquiry() {
  const nameId = useId();
  ...
  // Broadcasts focus state on the form so `NaturalBirthMobileEnquiryBar`
  // can hide itself while a field is actively being edited — mirrors
  // `vbac-enquiry.tsx`'s own effect.
  useEffect(() => {
  ...
  return (
    <section id="enquiry" aria-labelledby="natural-birth-enquiry-heading" className="relative isolate scroll-mt-[var(--header-height)] bg-[#e8cec2] section-pad">
      <NaturalBirthReveal className="container-birthwave grid gap-12 lg:grid-cols-[1fr_1fr] lg:items-start lg:gap-20">
        <div className="max-w-md">
          <p className="eyebrow">{naturalBirthEnquiry.eyebrow}</p>
          <h2 id="natural-birth-enquiry-heading" className="mt-4 text-[clamp(2rem,1.5rem+2.6vw,3.4rem)] leading-[1.1] font-semibold tracking-[var(--tracking-tight)] text-ink">
            {naturalBirthEnquiry.heading}
          </h2>
          <p className="mt-5 text-lg leading-[var(--leading-relaxed)] text-ink-soft">{naturalBirthEnquiry.body}</p>
          <p className="mt-8 border-t border-[var(--color-border)] pt-4 text-sm leading-relaxed text-ink-soft">{naturalBirthEnquiry.urgentNote}</p>
        </div>

        <div className="w-full max-w-md rounded-sm bg-paper p-7 sm:p-8">
          <p className="mb-5 font-body text-xs font-semibold tracking-[var(--tracking-wider)] text-terracotta-deep uppercase">{naturalBirthEnquiry.previewNotice}</p>

          {status === "previewed" ? (
            <p role="status" className="text-base leading-[var(--leading-relaxed)] text-ink-soft">
              {naturalBirthEnquiry.previewResult}
            </p>
          ) : (
            <form ref={formRef} onSubmit={handleSubmit} noValidate className="flex flex-col gap-5">
              <div className="flex flex-col gap-1.5">
                <label htmlFor={nameId} className={labelClass}>
                  {naturalBirthEnquiry.fields.name}
                </label>
                <input
                  id={nameId}
                  name="name"
              ...
              <div className="flex flex-col gap-1.5">
                <label htmlFor={contactId} className={labelClass}>
                  {naturalBirthEnquiry.fields.contact}
                </label>
                <input
                  id={contactId}
                  name="contact"
              ...
              <div className="flex flex-col gap-1.5">
                <label htmlFor={messageId} className={labelClass}>
                  {naturalBirthEnquiry.fields.message}
                </label>
              ...
              <button type="submit" className={submitButtonClass}>
                {naturalBirthEnquiry.submitLabel}
              </button>
            </form>
          )}
        </div>
 ...
 * focus. Mirrors `VbacMobileEnquiryBar` exactly, retargeted at this
 * page's own `#natural-birth-hero`/`#enquiry` ids.
 */
export function NaturalBirthMobileEnquiryBar() {
  const [pastHero, setPastHero] = useState(false);
  const [formVisible, setFormVisible] = useState(false);
  const [formEditing, setFormEditing] = useState(false);

  useEffect(() => {
    const hero = document.getElementById("natural-birth-hero");
  ...
  return (
    ...
    >
      <a
        href={naturalBirthMobileCta.href}
