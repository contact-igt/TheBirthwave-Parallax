 ...
 * one-shot reveal as `NutritionReveal`/`PostpartumReveal`. `useScrollReveal`
 ...
 * permanently invisible. The `newborn-reveal` class plus the single
 * `<noscript>` override in
 * `newborn-pediatric-care-landing-page.tsx` forces every instance back to
 * its settled, fully visible state when JS is unavailable.
 */
export function NewbornReveal({ className, children }: { className?: string; children: ReactNode }) {
  ...
  return (
    <div
      ref={ref}
      className={cx(
        "newborn-reveal transition-[opacity,transform] duration-[var(--duration-base)] ease-[var(--ease-signature)] motion-reduce:translate-y-0 motion-reduce:opacity-100",
